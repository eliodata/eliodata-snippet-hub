...
import { SnippetEmbedding, SnippetMetadata, VectorSearchResult, VectorIndex, ChunkData, VectorCacheConfig, VectorCacheDiagnostic } from '../types/Vector';
import { Snippet } from '../types/Snippet';
import { PHPAnalyzer } from '../utils/PHPAnalyzer';
...
export class VectorService {
     ...
     */
    async vectorizeSnippet(snippet: Snippet, forceInactive: boolean = false): Promise<void> {
        try {
            ...
            const metadata: SnippetMetadata = {
                functions: analysisResult.functions.map((f: any) => f.name),
                hooks: analysisResult.hooks.map((h: any) => h.name),
                ...
                classes: analysisResult.classes.map((c: any) => c.name),
                constants: analysisResult.constants.map((c: any) => c.name),
     ...
     */
    async searchSimilar(query: string, limit: number = 10, includeInactive: boolean = false): Promise<VectorSearchResult[]> {
        try {
     ...
     * Créer le texte combiné pour la vectorisation
     */
    private createCombinedText(snippet: Snippet, metadata: SnippetMetadata): string {
        const parts = [
            snippet.name,
            snippet.description,
            snippet.code,
     ...
     */
    async cleanupInactiveVectors(activeSnippetIds: string[]): Promise<number> {
        let cleanedCount = 0;
     ...
     */
    diagnoseVectorCache(): VectorCacheDiagnostic {
     ...
     */
    async testVectorSearch(query: string): Promise<{
        queryVector: number[];
        ...
        return {
            queryVector: queryVector.slice(0, 10), // Premiers 10 éléments pour debug
            vocabularyUsed,
            results,
            searchTime
        };
