...
class SnippetController {
    ...
    async deleteSnippet(snippet) {
        ...
        const confirm = await vscode.window.showWarningMessage(`Supprimer le snippet "${snippet.name}" ?`, { modal: true }, 'Oui');
        if (confirm === 'Oui') {
    ...
    async editAttribution(item) {
        ...
        const selectedMode = await vscode.window.showQuickPick([
            { label: 'Tous les contenus', value: 'all' },
        ...
        const success = await this.snippetProvider.updateAttribution(snippet.id, selectedMode.value, targetPostTypes, targetPostIds);
        if (success) {
